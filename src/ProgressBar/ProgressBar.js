...
export default class ProgressBar {
  ...
  init() {
    ...
    this._animateElement.getElementsByTagName("input")[0].checked =
      this.animated;
    this._onAnimate(this.animated);

    this._hideElement.getElementsByTagName("input")[0].checked = this.hided;
    this._onHide(this.hided);
  }
  ...
  _createCircle(type) {
    ...
    if (type === "progress") {
      const strokeDasharray = 2 * this._circleOptions.r * Math.PI + 4;
  ...
  _setValue(value = this.value) {
    this._validateInput(parseInt(value));

    if (!this._isValidValue) return;

    this.value = parseInt(value);
    if (!this.__currentAnimationId && !this.animated) {
      this._animate();
    }
  }
  ...
  _onHide(value) {
    ...
    value
      ...
      : (barElement.style.visibility = null);
  }

  _animate() {
    let currentValue = 0;
    let finishValue = this.value;
    ...
    const start = () => {
      ...
      if (currentValue === finishValue && !this.animated) {
        window.cancelAnimationFrame(this.__currentAnimationId);
        this.__currentAnimationId = null;

        if (this.value !== finishValue) this._animate();
      }
